...
from slide_analysis_nn.train.datasets_preparation.settings import TRAIN_DIR_NAME, TEST_DIR_NAME
from slide_analysis_nn.train.settings import SNAPSHOTS_DIR, BATCH_SIZE

training_patches_tumor = pd.DataFrame(columns=['patch_path', 'is_tumor'])
tumor_patch_folder = TRAIN_DIR_NAME / 'Tumor'
...
normal_patch_folder_i = TRAIN_DIR_NAME / 'Background'
normal_patch_paths_i = glob.glob(osp.join(normal_patch_folder_i, '*.png'))
normal_patch_paths = normal_patch_paths_i
...
vtumor_patch_folder = TEST_DIR_NAME / 'Tumor'
...
vnormal_patch_folder_i = TEST_DIR_NAME / 'Background'
vnormal_patch_paths_i = glob.glob(osp.join(vnormal_patch_folder_i, '*.png'))
vnormal_patch_paths = vnormal_patch_paths_i
...
def concatenated_block(x, specs, channel_axis, name):
    ...
    branch_3 = MaxPooling2D((3, 3), strides=(1, 1), padding='same', name=name + "_Branch_3_a_max")(
        x)
...
def InceptionV1(include_top=True,
    ...
    if include_top:
        ...
        x = Conv2D(classes, (1, 1), strides=(2, 2), padding='valid', use_bias=True, name='Logits')(
            x)
...
filepath = str(SNAPSHOTS_DIR / "googlenet_{epoch:02d}_{val_acc:.2f}.h5")
...
plt.legend(['train', 'validation'], loc='upper left')
plt.show()
plt.savefig('./Loss_plot_googlenet.png')
